...
QeITH <- function(data, type = c('bulk', 'single-cell','spatial')) {
  ...
  if (type == "bulk") {
    ...
    result <- data magrittr::%>%
      dplyr::group_by(Samples, Celltype) magrittr::%>%
      dplyr::summarise(count = n(), .groups = 'drop')

    # Convert to wide format (sample × cell type matrix)
    result2 <- result magrittr::%>%
      tidyr::pivot_wider(names_from = Celltype, values_from = count, values_fill = list(count = 0)) magrittr::%>%
      as.data.frame()
